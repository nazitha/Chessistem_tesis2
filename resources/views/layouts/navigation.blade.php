@php
    ...
    // Debug de permisos
    Log::info('Vista navigation: Verificando permisos', [
        'can_view_usuarios' => PermissionHelper::canViewModule('usuarios'),
...
<nav x-data="{ open: false }" class="bg-gray-800 border-b border-gray-700">
        ...
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ml-10 sm:flex">
                    ...
                    @php
                        $canViewUsuarios = PermissionHelper::canViewModule('usuarios');
                        $canViewTorneos = PermissionHelper::canViewModule('torneos');
                        
                        // Debug de permisos
                        Log::info('Navigation: Verificando permisos', [
                            'can_view_usuarios' => $canViewUsuarios,
                            'can_view_torneos' => $canViewTorneos
                        ]);
                    @endphp
                    ...
                    @if($canViewTorneos)
                        <x-nav-link :href="route('torneos.index')" :active="request()->routeIs('torneos.*')" class="text-gray-300 hover:text-white">
                            {{ __('Torneos') }}
                        </x-nav-link>
                    @endif
                </div>
                        ...
                        <x-dropdown align="right" width="60">
                            <x-slot name="trigger">
                                <span class="inline-flex rounded-md">
                                    <button type="button" class="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md text-gray-300 bg-gray-800 hover:text-white hover:bg-gray-700 focus:outline-none focus:bg-gray-700 active:bg-gray-700 transition ease-in-out duration-150">
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden bg-gray-800">
        <div class="pt-2 pb-3 space-y-1">
            ...
            @if($canViewUsuarios)
                <x-responsive-nav-link :href="route('usuarios.index')" :active="request()->routeIs('usuarios.*')" class="text-gray-300 hover:text-white">
                    {{ __('Usuarios') }}
                </x-responsive-nav-link>
            @endif

            @if($canViewTorneos)
                <x-responsive-nav-link :href="route('torneos.index')" :active="request()->routeIs('torneos.*')" class="text-gray-300 hover:text-white">
                    {{ __('Torneos') }}
                </x-responsive-nav-link>
            @endif
        </div>
        ...
        <div class="pt-4 pb-1 border-t border-gray-700">
            ...
            <div class="mt-3 space-y-1">
                ...
                @if (Laravel\Jetstream\Jetstream::hasTeamFeatures())
                    ...
                    @can('create', Laravel\Jetstream\Jetstream::newTeamModel())
                        <x-responsive-nav-link :href="route('teams.create')" :active="request()->routeIs('teams.create')" class="text-gray-300 hover:text-white">
                            {{ __('Create New Team') }}
                        </x-responsive-nav-link>
                    @endcan
